...
SELECT 'COMPLETADA',
       '2026-05-10 09:00:00',
       (SELECT `id` FROM `doctores` WHERE `email` = 'laura.gomez@medflow.com'),
       (SELECT `id` FROM `pacientes` WHERE `documento` = 'CC-1001001001'),
       (SELECT `id` FROM `procedimientos` WHERE `nombre` = 'Consulta General')
WHERE NOT EXISTS (
  SELECT 1
  FROM `citas` c
...
SELECT 'PROGRAMADA',
       '2026-05-12 09:00:00',
       (SELECT `id` FROM `doctores` WHERE `email` = 'laura.gomez@medflow.com'),
       (SELECT `id` FROM `pacientes` WHERE `documento` = 'CC-1001001002'),
       (SELECT `id` FROM `procedimientos` WHERE `nombre` = 'Consulta General')
WHERE NOT EXISTS (
  SELECT 1
  FROM `citas` c
...
SELECT 'PROGRAMADA',
       '2026-05-12 11:00:00',
       (SELECT `id` FROM `doctores` WHERE `email` = 'andres.ruiz@medflow.com'),
       (SELECT `id` FROM `pacientes` WHERE `documento` = 'CC-1001001003'),
       (SELECT `id` FROM `procedimientos` WHERE `nombre` = 'Control Dermatologico')
WHERE NOT EXISTS (
  SELECT 1
  FROM `citas` c
  JOIN `doctores` d ON d.`id` = c.`doctor_id`
...
SELECT 'CANCELADA',
       ...
       (SELECT `id` FROM `doctores` WHERE `email` = 'laura.gomez@medflow.com'),
       (SELECT `id` FROM `pacientes` WHERE `documento` = 'CC-1001001003'),
       (SELECT `id` FROM `procedimientos` WHERE `nombre` = 'Valoracion Prioritaria')
WHERE NOT EXISTS (
  SELECT 1
  FROM `citas` c
  JOIN `doctores` d ON d.`id` = c.`doctor_id`
...
WHERE NOT EXISTS (
  ...
  JOIN `doctores` d ON d.`id` = e.`doctor_id`
  WHERE d.`email` = 'laura.gomez@medflow.com'
